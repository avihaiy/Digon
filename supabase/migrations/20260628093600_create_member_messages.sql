...
CREATE TABLE public.member_messages (
    ...
    member_id uuid REFERENCES public.members(id) ON DELETE CASCADE, -- null means global message
    title text,
...
-- Policy for Managers (Admin)
CREATE POLICY "Managers can do everything on member_messages"
...
-- Policy for Anonymous (if they need to mark as read from the client side)
-- Since they are not authenticated via Supabase auth, we allow anonymous update ONLY for is_read flag
CREATE POLICY "Allow public to mark as read"
    ON public.member_messages
    AS PERMISSIVE
    FOR UPDATE
    TO public
    USING (true)
    WITH CHECK (true);
-- Note: It's better to create an RPC function for marking as read to keep it secure, but for simplicity this works if they just update `is_read`. Actually let's restrict the update to just is_read in the app, or create an RPC.

-- Create RPC function to mark a message as read (bypasses RLS safely)
...
-- Update `get_member_area_data` to include messages
