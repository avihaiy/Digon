  ...
  return (
    ...
    >
      ...
      <div className="mb-[1.5vh]">
        ...
        <p className="text-[1.8vh] md:text-[2.2vh] mt-[0.3vh] text-amber-200/60">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {pageNames.length > 0 ? pageNames.map((name, index) => (
            <motion.div
              key={name.id}
            ...
            >
              ...
              <div className="flex-shrink-0 py-[0.5vh]">
                <AnimatedTallCandle uniqueId={`r-${name.id}`} />
              </div>
              ...
              >
                <p className="text-[2.6vh] md:text-[3vh] font-bold leading-tight text-amber-100">
                  {name.is_male !== false ? "ר'" : "מרת"} {name.deceased_name} ז״ל
                </p>
                <p className="text-[1.6vh] md:text-[1.8vh] mt-[0.2vh] text-amber-200/70">
                  {name.is_male !== false ? "בן" : "בת"} ר' {name.father_name}
                </p>
                <p className="text-[1.4vh] md:text-[1.6vh] mt-[0.3vh] font-semibold text-amber-400">
                  {gematriya(name.hebrew_death_day)} {HEBREW_MONTH_NAMES[name.hebrew_death_month] || ''}
                </p>
              </div>

              {/* Left candle */}
              <div className="flex-shrink-0 py-[0.5vh]">
                <AnimatedTallCandle uniqueId={`l-${name.id}`} />
              </div>
            </motion.div>
          )) : (
            <p className="text-[2.5vh] text-amber-200/50">אין נפטרים ברשימה</p>
          )}
        </motion.div>
      </AnimatePresence>
      ...
      <div className="mt-[1.5vh]">
        ...
        <p className="text-[1.6vh] text-amber-300/50 font-semibold">
