  ...
  return (
    <div className="space-y-6">
      ...
      {(recurringReminders.length > 0 || dueRemindersCount > 0) && (
       ...
       <Card className="glass-card relative overflow-hidden border-0 shadow-md">
          <div className="absolute top-0 right-0 w-24 h-24 bg-amber-500/5 rounded-full -translate-y-12 translate-x-12 pointer-events-none" />
          <CardHeader className="pb-1 pt-3 px-3 sm:px-4 relative z-10">
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-1.5">
                <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-lg bg-amber-500/15 flex items-center justify-center shrink-0">
                  <Bell className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-amber-500" />
                </div>
                <CardTitle className="text-sm sm:text-base font-bold tracking-tight">תזכורות חוזרות</CardTitle>
                {dueRemindersCount > 0 && (
                  <Badge variant="destructive" className="animate-pulse text-[9px] sm:text-[10px] h-4 px-1">
                    {dueRemindersCount}
                  </Badge>
                )}
              </div>
              <Link to="/reminders">
                <Button variant="ghost" size="sm" className="text-[10px] sm:text-xs gap-0.5 h-6 px-1.5">
                  הכל
          ...
          <CardContent className="relative z-10 px-3 sm:px-4 pb-3 pt-1">
            {remindersLoading ? (
              <Skeleton className="h-10 w-full" />
            ) : recurringReminders.length === 0 ? (
              <p className="text-center text-muted-foreground py-2 text-xs">אין תזכורות חוזרות פעילות</p>
            ) : (
              <div className="space-y-1">
                {recurringReminders.slice(0, 4).map((r: any) => {
                  ...
                  return (
                    <div
                      key={r.id}
                      className={cn(
                        'flex items-center gap-1.5 sm:gap-2 py-1.5 px-2 rounded-lg transition-colors text-xs',
                        isPast ? 'bg-destructive/5 border border-destructive/20' : 'bg-muted/30 hover:bg-muted/50',
                        r.is_important && !isPast && 'border border-amber-400/30 bg-amber-50/15 dark:bg-amber-950/10'
                      )}
                    >
                      {r.is_important ? (
                        <Star className="w-3 h-3 fill-amber-400 text-amber-400 shrink-0" />
                      ) : (
                        <Repeat className={cn('w-3 h-3 shrink-0', isPast ? 'text-destructive' : 'text-primary')} />
                      )}
                      <p className={cn('font-medium truncate flex-1 text-[11px] sm:text-xs', r.is_important && 'text-amber-700 dark:text-amber-300')}>{r.content}</p>
                ...
                {recurringReminders.length > 4 && (
                  <Link to="/reminders" className="block text-center">
                    <Button variant="link" size="sm" className="text-[10px] h-5 py-0">
                      +{recurringReminders.length - 4} נוספות
                    </Button>
                  </Link>
                )}
              </div>
