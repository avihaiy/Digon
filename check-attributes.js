import { Client, Databases } from 'node-appwrite';
const client = new Client()
    .setEndpoint('https://fra.cloud.appwrite.io/v1')
    .setProject('6a6748ef003255232493')
    .setKey('standard_');
const databases = new Databases(client);

async function run() {
    const res = await databases.listAttributes('6a674a380025507f9db2', '6a674a390030977cf6ae');
    console.log(res.attributes.filter(a => ['flare', 'radar_unlock'].includes(a.key)).map(a => `${a.key}: ${a.status}`).join('\n'));
}
run();
